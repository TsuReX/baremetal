...
void usb_send_tog_set(uint32_t send_tog)
{
	/* Read HCTL register */
	spi_chip_activate();
...
int16_t usb_bulk_receive(uint8_t dev_addr, uint8_t ep_addr, void* dst_buf, size_t buf_size)
{
	/* Set device address */
	spi_chip_activate();
	peraddr_write(dev_addr);
...
void usb_recv_tog_set(uint32_t recv_tog)
{
	/* Read HCTL register */
	spi_chip_activate();
	uint8_t hctl = hctl_read();
...
void kb_usb_setup_get_conf(uint8_t dev_addr)
{
	...
	int16_t hrslt = usb_setup_send(dev_addr, &get_ep_status);
	...
	hrslt = usb_bulk_receive(dev_addr, 0, &configuration, get_ep_status.w_length);
...
//	mdelay(10);
	...
	int16_t hrslt = usb_setup_send(dev_addr, &get_dev_descr);
	...
	hrslt = usb_setup_send(dev_addr, &get_dev_descr);
...
void kb_usb_setup_get_conf_descr(uint8_t dev_addr)
{
	...
	hrslt = usb_bulk_receive(dev_addr, 0, &conf_descr, sizeof(conf_descr));
...
void kb_usb_setup_get_ep_status(uint8_t dev_addr, uint8_t ep_addr)
{
	...
	hrslt = usb_bulk_receive(dev_addr, 0, status, get_ep_status.w_length);
...
void kb_usb_setup_get_full_conf(uint8_t dev_addr)
{
	...
	int16_t ret_val = usb_setup_send(dev_addr, &get_dev_descr);
	...
	ret_val = usb_bulk_receive(dev_addr, 0, &conf_descr, get_dev_descr.w_length);
	...
	ret_val = usb_setup_send(dev_addr, &get_dev_descr);
	...
	ret_val = usb_bulk_receive(dev_addr, 0, configuration, get_dev_descr.w_length );
	...
	while (remain_bytes > 1) {
		...
		case 0x2: /* CONFIGURATION_DESCRIPTOR_TYPE */
			...
			d_print("b_length: 0x%02X\r\n", p_conf_descr->b_length);
			d_print("b_descriptor_type: 0x%02X\r\n", p_conf_descr->b_descriptor_type);
			d_print("w_total_length: 0x%04X\r\n", p_conf_descr->w_total_length);
			d_print("b_num_interfaces: 0x%02X\r\n", p_conf_descr->b_num_interfaces);
			d_print("b_configuration_value: 0x%02X\r\n", p_conf_descr->b_configuration_value);
			d_print("i_configuration: 0x%02X\r\n", p_conf_descr->i_configuration);
			d_print("bm_attributes: 0x%02X\r\n", p_conf_descr->bm_attributes);
			d_print("b_max_power: 0x%02X\r\n", p_conf_descr->b_max_power);
			mdelay(200);
			break;
		...
		case 0x4: /* INTERFACE_DESCRIPTOR_TYPE */
			...
			d_print("b_length: 0x%02X\r\n", p_iface_descr->b_length);
			d_print("b_descriptor_type: 0x%02X\r\n", p_iface_descr->b_descriptor_type);
			d_print("b_interface_number: 0x%02X\r\n", p_iface_descr->b_interface_number);
			d_print("b_alternate_setting: 0x%02X\r\n", p_iface_descr->b_alternate_setting);
			d_print("b_num_endpoints: 0x%02X\r\n", p_iface_descr->b_num_endpoints);
			d_print("b_interface_class: 0x%02X\r\n", p_iface_descr->b_interface_class);
			d_print("b_interface_subclass: 0x%02X\r\n", p_iface_descr->b_interface_subclass);
			d_print("b_interface_protocol: 0x%02X\r\n", p_iface_descr->b_interface_protocol);
			d_print("i_interface: 0x%02X\r\n", p_iface_descr->i_interface);
			mdelay(200);
			break;
		...
		case 0x5: /* ENDPOINT_DESCRIPTOR_TYPE */
			...
			d_print("b_length: 0x%02X\r\n", p_endp_descr->b_length);
			d_print("b_descriptor_type: 0x%02X\r\n", p_endp_descr->b_descriptor_type);
			d_print("b_endpoint_address: 0x%02X\r\n", p_endp_descr->b_endpoint_address);
			d_print("bm_attributes: 0x%02X\r\n", p_endp_descr->bm_attributes);
			d_print("w_max_packet_size: 0x%02X\r\n", p_endp_descr->w_max_packet_size);
			d_print("b_interval: 0x%02X\r\n", p_endp_descr->b_interval);
			mdelay(200);
			break;
