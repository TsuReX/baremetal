...
unsigned int uart_platform_base(unsigned int idx) {
...
void uart_sio_init(unsigned int idx) {
    ...
    unsigned int	ComBase = uart_platform_base(idx);
...
void uart_tx_byte(unsigned int idx, unsigned char data) {
    ...
    while (i-- && !(inb(base_port + UART8250_LSR) & UART8250_LSR_THRE));
...
unsigned char uart_rx_byte(unsigned int idx) {
    ...
    while (i-- && !(inb(base_port + UART8250_LSR) & UART8250_LSR_DR));
